...
namespace UniversalNameGenerator.Controllers
{
	...
	public class CategoryController
	{
		...
		public string GenerateName(string id)
		{
			...
			while (!NameIsValid(name, filters))
			{
				...
				while (name.Contains("{") || name.Contains("}"))
				{
					...
					{
						case "random":
							value = RandomString(split[1], int.Parse(split[2]), int.Parse(split[3]));
							break;
					}
		...
		string RandomString(string charlist, int minLength, int maxLength)
		{
			...
			{
				int i = rnd.Next(0, charlist.Length);
				str += charlist[i];
			}
